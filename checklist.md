...
## 2026-07-09 게임 나가기 버튼 + Memory Match 카드 크기 (Fable 직접 수정)
- [x] Memory Match 카드 이모지 크기 확대 (`.mmCard` font-size: 1.8rem → `clamp(2rem,9vw,2.8rem)`)
- [x] 5개 게임(테트리스/벽돌깨기/Word-Rocket/Memory Match/Speed Tap) 전부에 플레이 중 나가기(←) 버튼 추가 — 각 게임의 기존 `endX()` 정리 로직(타이머/RAF 정지)만 재사용하고 "Time's Up" 연출 없이 곧바로 게임 선택 화면으로 복귀
- [x] `node --check` 통과
- [x] Chrome 확장 도구로 직접 검증: 구구단 만점 실제 플레이 → Memory Match 진입 → 이모지 확대(개구리 카드 확대 스크린샷) 확인 → 나가기 버튼 클릭 시 게임 선택 화면 복귀 확인 → 테트리스에서도 나가기 버튼 동일하게 동작 확인. 콘솔 에러 0건(Chrome 확장 자체의 무관한 메시징 에러 제외).
